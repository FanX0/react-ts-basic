...
const CrudEasyLocal = () => {
  ...
  const onCreate = () => {
    ...
    const next = local.add({ name, description } as Omit<Destination, "id">);
    setItems(next);
    setName("");
    setDescription("");
  };
  ...
  const onUpdate = () => {
    ...
    const next = local.update(editingId, { name, description });
    setItems(next);
    setEditingId(null);
    setName("");
    setDescription("");
